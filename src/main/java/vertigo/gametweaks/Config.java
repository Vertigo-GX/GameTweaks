	...
	public boolean read() {
		...
		try(BufferedReader reader = new BufferedReader((new FileReader(file)))) {
			...
			while((line = reader.readLine()) != null) {
				...
				switch(segments[0]) {
					case DISABLE_LIGHTNING_STARTS_FIRES ->
							disableLightningStartsFires = segments[1].equals("true");
					case DISABLE_PORTALS_SPAWN_PIGLINS ->
							disablePortalsSpawnPiglins = segments[1].equals("true");
					case DISABLE_ENDERMEN_PICK_UP_BLOCKS ->
							disableEndermenPickUpBlocks = segments[1].equals("true");
				}
			}
		} catch(IOException e) {
